...
class Model(object):
    def __init__(self, config, batch, word_mat=None, char_mat=None, trainable=True, opt=True):
        ...
        self.c, self.q, self.ch, self.qh, self.y1, self.y2, self.qa_id, self.c_elmo, self.q_elmo = batch.get_next()
        ...
        if opt:
            ...
            self.c_elmo = tf.slice(self.c_elmo, [0, 0, 0], [N, self.c_maxlen+2, 50])
            self.q_elmo = tf.slice(self.q_elmo, [0, 0, 0], [N, self.q_maxlen+2, 50])
        ...
        self.elmo = BidirectionalLanguageModel(config.elmo_options_file, config.elmo_weight_file)
        ...
        if trainable:
            ...
            # Introduce Update Cycles
            _variables = cycle.replicate_variables(variables)
            zero_op = cycle.zero_variables(_variables)
            collect_op = cycle.collect_gradients(gradients, _variables)
            gradients = cycle.scale_gradients(_variables, 1. / config.cycle)

            self.zero_op = zero_op
            self.collect_op = collect_op
    ...
    def ready(self):
        ...
        with tf.variable_scope("emb"):
            ...
            c_elmo_emb = weight_layers(
                'embedding', c_elmo_features, l2_coef=0.0, do_layer_norm=False)['weighted_op']
            tf.get_variable_scope().reuse_variables()
            q_elmo_emb = weight_layers(
                'embedding', q_elmo_features, l2_coef=0.0, do_layer_norm=False)['weighted_op']

            c_elmo_emb = dropout(c_elmo_emb, keep_prob=config.elmo_keep_prob, is_train=self.is_train)
            q_elmo_emb = dropout(q_elmo_emb, keep_prob=config.elmo_keep_prob, is_train=self.is_train)
        ...
        with tf.variable_scope("match"):
            ...
            c_elmo_enc = weight_layers(
                'encoding', c_elmo_features, l2_coef=0.0, do_layer_norm=False)['weighted_op']
            tf.get_variable_scope().reuse_variables()
            q_elmo_enc = weight_layers(
                'encoding', q_elmo_features, l2_coef=0.0, do_layer_norm=False)['weighted_op']

            c_elmo_enc = dropout(c_elmo_enc, keep_prob=config.elmo_keep_prob, is_train=self.is_train)
            q_elmo_enc = dropout(q_elmo_enc, keep_prob=config.elmo_keep_prob, is_train=self.is_train)
