...
# include batcher to convert input tokens into elmo tokens
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False, elmo=None):
    ...
    for example in tqdm(examples):
        ...
        elmo_context_ids = elmo.one_sentence(example["context_tokens"], para_limit)
        elmo_ques_ids = elmo.one_sentence(example["ques_tokens"], ques_limit)
        ...
        record = tf.train.Example(features=tf.train.Features(feature={
                                  ...
                                  "elmo_context_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[elmo_context_ids.tostring()])),
                                  "elmo_ques_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[elmo_ques_ids.tostring()])),
...
def prepro(config):
    print("Loading Elmo Vocabulary")
    elmo_batcher = Batcher(config.elmo_vocab, 50)
    ...
    build_features(config, train_examples, "train",
                   config.train_record_file, word2idx_dict, char2idx_dict, elmo=elmo_batcher)
    dev_meta = build_features(config, dev_examples, "dev",
                              config.dev_record_file, word2idx_dict, char2idx_dict, elmo=elmo_batcher)
    test_meta = build_features(config, test_examples, "test",
                               config.test_record_file, word2idx_dict, char2idx_dict, is_test=True, elmo=elmo_batcher)
