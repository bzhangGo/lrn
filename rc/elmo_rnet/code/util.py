    ...
    def parse(example):
        ...
        features = tf.parse_single_example(example,
                                           features={
                                               ...
                                               "elmo_context_idxs": tf.FixedLenFeature([], tf.string),
                                               "elmo_ques_idxs": tf.FixedLenFeature([], tf.string),
        ...
        elmo_context_idxs = tf.reshape(tf.decode_raw(
            features["elmo_context_idxs"], tf.int32), [para_limit+2, 50])
        elmo_ques_idxs = tf.reshape(tf.decode_raw(
            features["elmo_ques_idxs"], tf.int32), [ques_limit+2, 50])
        y1 = tf.reshape(tf.decode_raw(
            features["y1"], tf.float32), [para_limit])
        y2 = tf.reshape(tf.decode_raw(
            features["y2"], tf.float32), [para_limit])
        qa_id = features["id"]
        return context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, y1, y2, qa_id, elmo_context_idxs, elmo_ques_idxs
    return parse
