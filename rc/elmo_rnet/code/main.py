...
def train(config):
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        step_counter, accum_loss = 0, 0.
        for _ in tqdm(range(1, config.num_steps + 1)):

            if step_counter == 0:
                sess.run(model.zero_op)

            loss, collect_op = sess.run([model.loss, model.collect_op], feed_dict={
                handle: train_handle})
            step_counter += 1
            accum_loss += loss

            if step_counter == config.cycle:
                global_step = sess.run(model.global_step) + 1
                train_op = sess.run([model.train_op], feed_dict={
                    handle: train_handle})
                step_counter = 0

                loss = accum_loss / config.cycle
                accum_loss = 0.
